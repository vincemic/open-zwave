...
)
{
	if( Node* node = GetNodeUnsafe() )
	{
		...
		switch( GetVersion() )
		{
			case 2:
			{
				node->CreateValueByte( ValueID::ValueGenre_System, GetCommandClassId(), _instance, SwitchBinaryIndex_Duration, "Transition Duration", "", false, false, 0xff, 0, endpoint );
				node->CreateValueBool( ValueID::ValueGenre_System, GetCommandClassId(), _instance, SwitchBinaryIndex_TargetState, "Target State", "", true, false, true, 0, endpoint );
				// Fall through to version 1
			}
			case 1:
			{
				node->CreateValueBool( ValueID::ValueGenre_User, GetCommandClassId(), _instance, SwitchBinaryIndex_Level, "Switch", "", false, false, false, 0, endpoint );
				break;
			}
		}
	}
